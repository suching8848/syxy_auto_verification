...
从 [Releases](https://github.com/suching8848/syxy_auto_verification/releases) 下载 `auto_login_v2.0.zip`，解压到任意文件夹（**注意整个文件夹一起解压，`assets` 子目录是图标，不能少**）。
...
- **注册计划任务**：任务名 `CampusNetAutoLogin`，按 `schedule_time` 触发（默认 20:30），运行 60 分钟后自动退出
- **任务配置**：`LogonType Interactive`（支持 browser 模式模拟按键）、`Hidden=$true`（不弹窗口）、2 小时执行时限、已有实例运行时忽略新实例

部署脚本读取配置中的 `schedule_time`（缺省 20:30）；修改配置后需要重新部署。也可显式覆盖：
...
分发给别人需要的文件（已打包在 `auto_login_v2.0.zip`）：
...
- 增加认证阶段计时、真实运行副本身份日志与白名单式日志脱敏；将已见过的会话冲突、门户参数错误等失败分开记录。
- 断网确认后的重试不再重复累计确认阈值；有限会话冲突快速重试仅在显式配置后启用，**默认关闭**。09-23 的单晚快速试用未观察到收益，因此不作为默认策略。
- 149 项离线测试通过。09-24 阶段 B 候选版在日常重新认证中 3 次尝试后自动恢复，首次失败探测至恢复约 12 秒；正式 v2.0 冻结版另经隔离静默冒烟，未在真实门户上单独复演认证。详见 `RELEASE_v2.0.md`。
...
- 修复 GUI 双路守护（v1.7.3 已知问题）：托盘不再自带探测线程，守护只归 `start_monitor()` / `stop_monitor()` 所有，「停止守护」是真的停。**打开界面后需要点「开始守护」才会开始探测**（这本来就是文档描述的行为）；`--tray` 等控制台托盘模式行为不变。
- 修复 UAC 取消判读：`ShellExecuteW` 只有返回值 >32 才算成功，失败原因要读 `GetLastError()`（取消 = 1223）。以前拿返回值直接比 1223，取消常常只报一句通用失败。
- 62 项离线测试通过。真机 UAC 交互与受控文件夹访问仍需人工复验，见 `RELEASE_v1.7.4.md` 的验证边界。
...
- 修复快速停止、重启时的守护线程重叠与旧状态覆盖；手动守护持续到用户停止。
- 每日计划任务到点立即检测，避免再次等待配置中的启动时间。
- 注册任务遇错即停止，核对任务路径、参数及触发时间，避免旧任务造成成功误报。
- 支持路径中的单引号，校验每日守护时长，发布说明纳入版本管理。
- 离线回归测试增至 39 项。升级后请重新部署每日任务。
...
- 构建 `CampusNet.exe` 后，运行 `python packaging/make_release_zip.py 2.0` 生成白名单发布包；说明文件随源码维护。源码修改不会自动更新已有 exe。
